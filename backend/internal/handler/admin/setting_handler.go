...
// GetModelMarketplaceData 获取模型广场数据
// GET /api/v1/admin/settings/model-marketplace
func (h *SettingHandler) GetModelMarketplaceData(c *gin.Context) {
	raw, err := h.settingService.GetModelMarketplaceData(c.Request.Context())
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// UpdateModelMarketplaceData 更新模型广场数据
// PUT /api/v1/admin/settings/model-marketplace
func (h *SettingHandler) UpdateModelMarketplaceData(c *gin.Context) {
	...
	if err := h.settingService.SetModelMarketplaceData(c.Request.Context(), string(body)); err != nil {
		response.ErrorFrom(c, err)
		return
	}
	response.Success(c, nil)
