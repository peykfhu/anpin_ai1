...
// GetModelMarketplaceData 获取模型广场公开定价数据（JSON 数组）
func (s *SettingService) GetModelMarketplaceData(ctx context.Context) (string, error) {
	value, err := s.settingRepo.GetValue(ctx, SettingKeyModelMarketplaceData)
	if err != nil {
		if errors.Is(err, ErrSettingNotFound) {
			return "[]", nil
		}
		return "", fmt.Errorf("get model marketplace data: %w", err)
	}
...
// SetModelMarketplaceData 保存模型广场公开定价数据（JSON 数组）
func (s *SettingService) SetModelMarketplaceData(ctx context.Context, data string) error {
	if !json.Valid([]byte(data)) {
		return fmt.Errorf("invalid JSON data")
	}
	return s.settingRepo.Set(ctx, SettingKeyModelMarketplaceData, data)
}
