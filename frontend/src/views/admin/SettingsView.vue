    ...
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.modelMarketplace.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.modelMarketplace.description') }}
            </p>
          </div>
          <div class="space-y-4 p-6">
            <div class="flex items-center justify-between">
              <span class="text-sm text-gray-700 dark:text-gray-300">
                {{ t('admin.settings.modelMarketplace.modelCount', { count: modelMarketplaceModels.length }) }}
              </span>
              <div class="flex gap-2">
                <button
                  type="button"
                  class="btn btn-secondary btn-sm"
                  @click="addMarketplaceModel"
                >
                  {{ t('admin.settings.modelMarketplace.addModel') }}
                </button>
              </div>
            </div>

            <div v-if="modelMarketplaceModels.length === 0" class="rounded-lg border border-dashed border-gray-300 p-6 text-center text-sm text-gray-400 dark:border-dark-600">
              {{ t('admin.settings.modelMarketplace.noModels') }}
            </div>

            <div v-for="(model, mIdx) in modelMarketplaceModels" :key="mIdx"
              class="rounded-lg border border-gray-200 p-4 dark:border-dark-600"
            >
              <div class="flex items-center justify-between mb-3">
                <span class="text-sm font-medium text-gray-900 dark:text-white">
                  {{ model.model || t('admin.settings.modelMarketplace.unnamed') }}
                </span>
                <button
                  type="button"
                  class="text-xs text-red-500 hover:text-red-700"
                  @click="modelMarketplaceModels.splice(mIdx, 1)"
                >
                  {{ t('common.delete') }}
                </button>
              </div>
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="text-xs text-gray-500">{{ t('admin.settings.modelMarketplace.modelName') }}</label>
                  <input v-model="model.model" type="text" class="input text-sm" placeholder="claude-sonnet-4-20250514" />
                </div>
                <div>
                  <label class="text-xs text-gray-500">{{ t('admin.settings.modelMarketplace.platform') }}</label>
                  <select v-model="model.platform" class="input text-sm">
                ...
                <div>
                  <label class="text-xs text-gray-500">{{ t('admin.settings.modelMarketplace.inputPrice') }}</label>
                  <input v-model.number="model.input_price" type="number" step="0.01" class="input text-sm" placeholder="3.00" />
                </div>
                <div>
                  <label class="text-xs text-gray-500">{{ t('admin.settings.modelMarketplace.outputPrice') }}</label>
                  <input v-model.number="model.output_price" type="number" step="0.01" class="input text-sm" placeholder="15.00" />
                </div>
                <div>
                  <label class="text-xs text-gray-500">{{ t('admin.settings.modelMarketplace.cacheWritePrice') }}</label>
                  <input v-model.number="model.cache_write_price" type="number" step="0.01" class="input text-sm" placeholder="3.75" />
                </div>
                <div>
                  <label class="text-xs text-gray-500">{{ t('admin.settings.modelMarketplace.cacheReadPrice') }}</label>
                  <input v-model.number="model.cache_read_price" type="number" step="0.01" class="input text-sm" placeholder="0.30" />
                </div>
                <div>
                  <label class="text-xs text-gray-500">{{ t('admin.settings.modelMarketplace.billingMode') }}</label>
                  <select v-model="model.billing_mode" class="input text-sm">
                    <option value="token">Token</option>
                    <option value="request">Per Request</option>
                  </select>
                </div>
                <div>
                  <label class="text-xs text-gray-500">{{ t('admin.settings.modelMarketplace.rateMultiplier') }}</label>
                  <input v-model.number="model.rate_multiplier" type="number" step="0.1" class="input text-sm" placeholder="1.0" />
                </div>
                <div class="col-span-2">
                  <label class="text-xs text-gray-500">{{ t('admin.settings.modelMarketplace.tags') }}</label>
                  <input v-model="model.tags_str" type="text" class="input text-sm" :placeholder="t('admin.settings.modelMarketplace.tagsPlaceholder')" />
                </div>
              </div>
            </div>
          </div>
...
interface MarketplaceModelEntry {
  model: string
  platform: string
  tags: string[]
  tags_str: string
...
const modelMarketplaceModels = reactive<MarketplaceModelEntry[]>([])

function addMarketplaceModel() {
  modelMarketplaceModels.push({
    model: '',
    platform: 'anthropic',
    tags: [],
    tags_str: '',
...
async function loadModelMarketplaceData() {
  try {
    const data = await adminAPI.settings.getModelMarketplaceData()
    modelMarketplaceModels.splice(0, modelMarketplaceModels.length)
    if (Array.isArray(data)) {
      for (const item of data) {
        const entry = item as Record<string, unknown>
        modelMarketplaceModels.push({
          model: (entry.model as string) || '',
          platform: (entry.platform as string) || 'anthropic',
          tags: Array.isArray(entry.tags) ? entry.tags as string[] : [],
          tags_str: Array.isArray(entry.tags) ? (entry.tags as string[]).join(', ') : '',
          input_price: entry.input_price as number | null,
          output_price: entry.output_price as number | null,
          cache_write_price: entry.cache_write_price as number | null,
          cache_read_price: entry.cache_read_price as number | null,
          billing_mode: (entry.billing_mode as string) || 'token',
          rate_multiplier: (entry.rate_multiplier as number) || 1.0,
        })
      }
    }
  } catch (err: unknown) {
...
async function saveModelMarketplaceData(): Promise<boolean> {
  try {
    const payload = modelMarketplaceModels.map((m) => ({
      model: m.model,
      platform: m.platform,
      tags: m.tags_str ? m.tags_str.split(',').map((t: string) => t.trim()).filter(Boolean) : m.tags,
      input_price: m.input_price,
      output_price: m.output_price,
      cache_write_price: m.cache_write_price,
      cache_read_price: m.cache_read_price,
      billing_mode: m.billing_mode,
      rate_multiplier: m.rate_multiplier || 1.0,
    }))
    await adminAPI.settings.updateModelMarketplaceData(payload)
    return true
  } catch (err: unknown) {
    appStore.showError(extractApiErrorMessage(err, t('common.error')))
    return false
...
async function loadSettings() {
  ...
  try {
    ...
    // Load model marketplace data
    await loadModelMarketplaceData()
  } catch (error: unknown) {
...
async function saveSettings() {
  ...
  try {
    ...
    // Save model marketplace data separately
    const mmOk = await saveModelMarketplaceData()
