      ...
      <template v-else>
        ...
        <template v-else>
          ...
          <template v-if="activeTab === 'recharge'">
            ...
            <template v-else>
            <!-- Recharge confirm (inline, replaces package grid) -->
            <template v-if="rechargeConfirmAmount !== null">
              <div class="card p-5">
                <div class="mb-3 flex items-center gap-2">
                  <span class="rounded-full bg-primary-500/10 px-2 py-0.5 text-xs font-medium text-primary-600 dark:text-primary-400">
                    {{ t('payment.recharge.badge') }}
                  </span>
                  <h3 class="text-lg font-bold text-gray-900 dark:text-white">
                    {{ rechargeConfirmAmount }}{{ t('payment.recharge.usdSuffix') }}
                  </h3>
                </div>
                <div class="flex items-baseline gap-2">
                  <span class="text-3xl font-bold text-primary-600 dark:text-primary-400">¥{{ totalAmount.toFixed(2) }}</span>
                </div>
                <p class="mt-2 text-sm text-green-600 dark:text-green-400">
                  +${{ creditedAmount.toFixed(2) }} {{ t('payment.recharge.balance') }}
                </p>
                <div v-if="balanceRechargeMultiplier !== 1" class="mt-2 text-xs text-gray-500 dark:text-gray-400">
              ...
              <button class="btn btn-secondary w-full" @click="rechargeConfirmAmount = null; amount = null">{{ t('common.cancel') }}</button>
            ...
            <!-- Recharge Package Cards -->
            <template v-else>
              <div :class="rechargeGridClass">
                <RechargePackageCard
                  v-for="pkg in rechargeAmounts"
                  :key="pkg"
                  :amount="pkg"
                  :multiplier="balanceRechargeMultiplier"
                  :fee-rate="feeRate"
                  @select="selectRechargePackage"
                />
              </div>
            </template>
            <div v-if="errorMessage && rechargeConfirmAmount === null" class="rounded-xl border border-red-200 bg-red-50 p-4 dark:border-red-800/50 dark:bg-red-900/20">
              <p class="text-sm text-red-700 dark:text-red-400">{{ errorMessage }}</p>
            </div>
            </template>
          </template>
...
import RechargePackageCard from '@/components/payment/RechargePackageCard.vue'
...
const rechargeConfirmAmount = ref<number | null>(null)

const rechargeAmounts = [10, 20, 50, 100, 200, 500, 1000, 2000, 5000]

function selectRechargePackage(amt: number) {
  rechargeConfirmAmount.value = amt
  amount.value = amt
  errorMessage.value = ''
}

const rechargeGridClass = computed(() => {
  const n = rechargeAmounts.length
